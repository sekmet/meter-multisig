...
1. In case that it is not already supported, add the network on the **METER_NETWORK** enum in [`src/config/networks/network.d.ts`](/src/config/networks/network.d.ts)
...
```typescript
import { METER_NETWORK, NetworkConfig } from 'src/config/networks/network.d'
